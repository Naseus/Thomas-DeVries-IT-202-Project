...
function moveTarget() {
  targetX = Math.round(Math.random() * canvas.width - targetLength / 2);
  targetY = Math.round(Math.random() * canvas.height - targetLength / 2)
}
...
function draw() {
  ...
  if (countdown <= 0 || y + sideLength > canvas.height || y < 0 || x < 0 || x + sideLength > canvas.width) {
    endGame();
  } else {
